  ...
  return (
    <motion.div
      ...
      animate={isHighlighted ? {
        boxShadow: [
          '0 0 0px rgba(254,240,138,0.8)',
          '0 0 20px rgba(254,240,138,1)',
          '0 0 0px rgba(254,240,138,0.8)'
        ],
        opacity: 1, y: 0
      } : { opacity: 1, y: 0 }}
      transition={isHighlighted ? { duration: 1, repeat: Infinity, repeatType: 'loop' } : {}} className={`z-0 relative w-full h-64 p-5 flex flex-col justify-center items-center bg-white rounded-2xl shadow-lg`}>
      <motion.div onClick={removeWidget} whileHover={{ scale: 1.08, color: '#f00' }}
        whileTap={{ scale: 0.95 }} className='absolute z-[5] top-3 right-5 w-6 h-6'><IoIosCloseCircleOutline className='w-full h-full cursor-pointer' /></motion.div>
